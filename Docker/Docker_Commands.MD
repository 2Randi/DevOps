...
```sh
docker run -i -t --name rocky_server rockylinux:9
i - interactif
t - Terminal
```
> Note : l'image `centos:latest` n'est plus maintenue depuis la fin de vie de CentOS Linux ; utilisez `rockylinux:9` ou `almalinux:9` comme équivalent.
